...
from helpers import highlight_differences
...
class PDF_Image_Compare:    
    ...
    def __init__(self,pdf1,pdf2):
        ...
        self.data_dir = self.download_dir +os.sep+ "data"
        self.pdf1 = pdf1
        self.pdf2 = pdf2
    ...
    def call_convert(self,src,dest):
        ...
        image_magick_path = "D:\ImageMagick-7.1.1-Q16-HDRI\magick.exe"
        try:
            # subprocess.check_call(["convert",src,dest], shell=True)
            if ".pdf" in src:
                subprocess.check_call([image_magick_path,"-density","300","-quality","300",src,dest], shell=True)
            else:
                subprocess.check_call([image_magick_path,src,dest], shell=True)
    ...
    def create_diff_image(self,pdf1_list,pdf2_list,diff_image_dir):
        ...
        for pdf1_img,pdf2_img in zip(pdf1_list,pdf2_list):
            ...
            try:
                pdf2_image = Image.open(self.download_dir +os.sep+"data"+os.sep+ pdf2_img)
                pdf1_image = Image.open(self.download_dir+os.sep + "data"+os.sep+ pdf1_img)
                diff = ImageChops.difference(pdf2_image,pdf1_image)
                # Convert the difference image to grayscale
                diff_image = diff.convert("L")
                ...
                # Draw a red rectangle around the differing region on the original image
                diffOverlayonPdf1 = ImageDraw.Draw(pdf2_image)
                diffOverlayonPdf1.rectangle(bbox, outline="red", width=2)

                # Save the result
                highlight_differences(self.download_dir+os.sep + "data"+os.sep+ pdf1_img,self.download_dir +os.sep+"data"+os.sep+ pdf2_img,diff_filename)
                # diff_image.save(diff_filename)
    ...
    def cleanup(self,diff_image_dir,pdf1_list,pdf2_list):
        ...
        for pdf1_img,pdf2_img in zip(pdf1_list,pdf2_list):
            try:
                print("**DELETE INTERMITTENT JPG**")
                # os.remove(self.download_dir +os.sep+"data"+os.sep+ pdf1_img)
                # os.remove(self.download_dir +os.sep+"data"+os.sep+ pdf2_img)
            except Exception as e:
